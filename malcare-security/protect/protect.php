...
if (!class_exists('MCProtect_V669')) :
...
class MCProtect_V669 {
	...
	public static function init($mode) {
		...
		if ($mode == MCProtect_V669::MODE_PREPEND) {
			$config_file = MCDATAPATH .  MCCONFKEY . '-' . 'mc.conf';
			$config = MCProtectUtils_V669::parseFile($config_file);

			if (empty($config['time']) || !($config['time'] > time() - (48*3600)) ||
					!isset($config['mc_conf_version']) ||
					(MCProtect_V669::CONF_VERSION !== $config['mc_conf_version'])) {
				return false;
			...
			$request = new MCProtectRequest_V669($request_ip_header, $req_config);
			$fw_config = array_key_exists('fw', $config) ? $config['fw'] : array();

			MCProtectFW_V669::getInstance($mode, $request, $fw_config, $brand_name)->init();
		} else {
			...
			MCProtectFW_V669::getInstance($mode, $request, $fw_config, $brand_name)->init();
			MCProtectLP_V669::getInstance($request, $lp_config, $brand_name)->init();
		}
	}
	...
	public static function uninstall() {
		...
		MCProtectIpstore_V669::uninstall();
		MCProtectFW_V669::uninstall();
		MCProtectLP_V669::uninstall();

		MCProtect_V669::removeWPPrepend();
		MCProtect_V669::removePHPPrepend();
		MCProtect_V669::removeMCData();
	...
	private static function removeWPPrepend() {
		...
		MCProtectUtils_V669::fileRemovePattern($fname, $pattern);
	}

	private static function removePHPPrepend() {
		MCProtect_V669::removeHtaccessPrepend();
		MCProtect_V669::removeUseriniPrepend();
	}

	private static function removeHtaccessPrepend() {
		$pattern = "/# MalCare WAF(.|\n)*# END MalCare WAF/i";

		MCProtectUtils_V669::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.htaccess", $pattern, true);
	}

	private static function removeUseriniPrepend() {
		$pattern = "/; MalCare WAF(.|\n)*; END MalCare WAF/i";

		MCProtectUtils_V669::fileRemovePattern(rtrim(ABSPATH, DIRECTORY_SEPARATOR) . "/.user.ini", $pattern, true);
	}
	...
	private static function removeMCData() {
		...
		MCProtectUtils_V669::rrmdir($mc_data_dir);
	}
}
endif;