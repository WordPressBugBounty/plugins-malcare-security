...
if (!trait_exists('MCProtectFWRuleWPFunc_V669')) :
trait MCProtectFWRuleWPFunc_V669 {
	private function _rf_sanitizeUser() {
		...
		if (!function_exists('sanitize_user') || !MCProtectUtils_V669::haveMupluginsLoaded()) {
			throw new MCProtectRuleError_V669(
	...
	private function _rf_maybeSerialize() {
		...
		if (!function_exists('maybe_serialize') || !MCProtectUtils_V669::haveMupluginsLoaded()) {
			throw new MCProtectRuleError_V669(
	...
	private function _rf_isUserLoggedIn() {
		...
		if (!function_exists('is_user_logged_in') || !MCProtectUtils_V669::havePluginsLoaded()) {
			throw new MCProtectRuleError_V669(
	...
	private function _rf_getCurrentWPUser() {
		...
		if (!function_exists('wp_get_current_user') || !MCProtectUtils_V669::havePluginsLoaded()) {
			throw new MCProtectRuleError_V669(
				$this->addExState("getCurrentWPUser: Func wp_get_current_user doesn't exist.")
			);
		}

		return MCProtectFWRuleEngine_V669::toAllowedType(wp_get_current_user());
	}
	...
	private function _rf_currentUserCan() {
		...
		if (!function_exists('current_user_can') || !MCProtectUtils_V669::havePluginsLoaded()) {
			throw new MCProtectRuleError_V669(
	...
	private function _rf_getUserBy() {
		...
		if (!function_exists('get_user_by') || !MCProtectUtils_V669::havePluginsLoaded()) {
			throw new MCProtectRuleError_V669(
		...
		return MCProtectFWRuleEngine_V669::toAllowedType($user);
	}
	...
	private function _rf_getCurrentWPUserCapabilities() {
		...
		return MCProtectFWRuleEngine_V669::toAllowedType($user["allcaps"]);
	}
	...
	private function _rf_getOption() {
		...
		if (!function_exists('get_option') || !MCProtectUtils_V669::haveMupluginsLoaded()) {
			throw new MCProtectRuleError_V669(
				$this->addExState("getOption: Func get_option doesn't exist.")
			);
		}

		return MCProtectFWRuleEngine_V669::toAllowedType(get_option($option, $default_value));
	}
	...
	private function _rf_checkPasswordResetKey() {
		...
		if (!function_exists('check_password_reset_key') || !MCProtectUtils_V669::havePluginsLoaded()) {
			throw new MCProtectRuleError_V669(
	...
	private function _rf_wpUnslash() {
		...
		if (!function_exists('wp_unslash') || !MCProtectUtils_V669::haveMuPluginsLoaded()) {
			throw new MCProtectRuleError_V669(
