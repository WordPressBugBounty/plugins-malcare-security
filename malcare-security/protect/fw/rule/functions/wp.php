...
if (!trait_exists('MCProtectFWRuleWPFunc_V676')) :
trait MCProtectFWRuleWPFunc_V676 {
	private function _rf_sanitizeUser() {
		...
		if (!function_exists('sanitize_user') || !MCProtectUtils_V676::haveMupluginsLoaded()) {
			throw new MCProtectRuleError_V676(
	...
	private function _rf_maybeSerialize() {
		...
		if (!function_exists('maybe_serialize') || !MCProtectUtils_V676::haveMupluginsLoaded()) {
			throw new MCProtectRuleError_V676(
	...
	private function _rf_isUserLoggedIn() {
		...
		if (!function_exists('is_user_logged_in') || !MCProtectUtils_V676::havePluginsLoaded()) {
			throw new MCProtectRuleError_V676(
	...
	private function _rf_getCurrentWPUser() {
		...
		if (!function_exists('wp_get_current_user') || !MCProtectUtils_V676::havePluginsLoaded()) {
			throw new MCProtectRuleError_V676(
				$this->addExState("getCurrentWPUser: Func wp_get_current_user doesn't exist.")
			);
		}

		return MCProtectFWRuleEngine_V676::toAllowedType(wp_get_current_user());
	}
	...
	private function _rf_currentUserCan() {
		...
		if (!function_exists('current_user_can') || !MCProtectUtils_V676::havePluginsLoaded()) {
			throw new MCProtectRuleError_V676(
	...
	private function _rf_getUserBy() {
		...
		if (!function_exists('get_user_by') || !MCProtectUtils_V676::havePluginsLoaded()) {
			throw new MCProtectRuleError_V676(
		...
		return MCProtectFWRuleEngine_V676::toAllowedType($user);
	}
	...
	private function _rf_getCurrentWPUserCapabilities() {
		...
		return MCProtectFWRuleEngine_V676::toAllowedType($user["allcaps"]);
	}
	...
	private function _rf_getOption() {
		...
		if (!function_exists('get_option') || !MCProtectUtils_V676::haveMupluginsLoaded()) {
			throw new MCProtectRuleError_V676(
				$this->addExState("getOption: Func get_option doesn't exist.")
			);
		}

		return MCProtectFWRuleEngine_V676::toAllowedType(get_option($option, $default_value));
	}
	...
	private function _rf_checkPasswordResetKey() {
		...
		if (!function_exists('check_password_reset_key') || !MCProtectUtils_V676::havePluginsLoaded()) {
			throw new MCProtectRuleError_V676(
	...
	private function _rf_wpUnslash() {
		...
		if (!function_exists('wp_unslash') || !MCProtectUtils_V676::haveMuPluginsLoaded()) {
			throw new MCProtectRuleError_V676(
