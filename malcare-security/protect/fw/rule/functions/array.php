...
if (!trait_exists('MCProtectFWRuleArrayFunc_V665')) :
trait MCProtectFWRuleArrayFunc_V665 {
	...
	private function keyMatchesRecursive($array, $pattern, &$traversed_keys, $depth = 1) {
		if ($depth > MCProtectFWRuleEngine_V665::MAX_DEPTH_TO_ALLOWED_TYPE_FUNC) {
			return false;
		}
	...
	private function enforceArrayTraversalLimit($func_name, $traversed_keys) {
		if ($traversed_keys > MCProtectFWRuleEngine_V665::MAX_ARRAY_KEYS_TO_TRAVERSE) {
			throw new MCProtectRuleError_V665(
				$this->addExState(
					"TraversalLimitError: " . $func_name . " exceeded " .
					MCProtectFWRuleEngine_V665::MAX_ARRAY_KEYS_TO_TRAVERSE .
					" array keys"
				)
			);
		}
	...
	private function _rf_digArrayValues() {
		...
		if (count($keys) > MCProtectFWRuleEngine_V665::MAX_DEPTH_TO_ALLOWED_TYPE_FUNC) {
			throw new MCProtectRuleError_V665(
				$this->addExState(
					"digArrayValues: Keys exceeded " .
					MCProtectFWRuleEngine_V665::MAX_DEPTH_TO_ALLOWED_TYPE_FUNC .
					" levels of traversal"
				)
			);
		}
		...
		foreach ($keys as $key) {
			...
			foreach ($nodes as $node) {
				...
				if ($key === MCProtectFWRuleEngine_V665::WILDCARD_KEY) {
